...
- [ ] **早上问 Siying 是否需要剔除特定 Amazon 类目**：例如 Kindle / books 等可能不应计入站点匹配或图搜范围的品类，确认剔除规则与列表【Brian】
- [ ] **精确 double confirm brand name 逻辑**：周一需和 Siying 确认 Amazon brand name 的精确匹配规则与输出口径，避免后续数据口径偏差【Brian】
